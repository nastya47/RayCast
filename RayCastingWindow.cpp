...
: Window(width, height)
{
	...
	_wall_texture = std::make_shared<Texture>(_renderer, "brick_texture1004.jpg");
}
...
void RayCastingWindow::render()
{
	...
	SDL_SetRenderDrawColor(_renderer.get(), 135, 206, 250, 255);
	SDL_RenderFillRect(_renderer.get(), &r_sky);
	SDL_SetRenderDrawColor(_renderer.get(), 0, 64, 0, 255);
